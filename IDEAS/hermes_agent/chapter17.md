...
chapter: 17
title: "Building the MVP — RE-AI v1 in 7-14 Days"
source: victoria-k-emails
---

# Chapter 17 — Building the MVP — RE-AI v1 in 7-14 Days
...
Incoming Lead Sources (Website / Email / SMS / Forms) │ ▼ Lead Intake Service │ ▼ RE-AI Orchestrator │ ┌──────────────┼──────────────┐ ▼ ▼ ▼ Lead Reply CRM Update Calendar Booking │ │ │ └──────────────┼──────────────┘ ▼ Telegram Approval / Alerts

🧱 2. Tech Stack (Simple, Practical) You already have most of this:
...
Logic:
New lead arrives → AI classifies intent → Generate response → Send via email/SMS → Log interaction → Update CRM status
...
If lead.status != closed AND no response in X days → send follow-up → log message

📅 MODULE 4: Calendar + Showing Scheduler Purpose:
Turn interest into booked showings.
Flow:
Client asks for viewing → AI suggests times → checks Google Calendar → books event → sends confirmation
...
Then route:
→ response generator → calendar handler → CRM updater

🗃️ 5. Database (Minimal Schema) Leads id name email phone message status (new / contacted / hot / cold / closed) created_at last_contacted

Interactions id lead_id type (sms/email/call) content timestamp

Appointments id lead_id datetime location status

🔌 6. n8n Workflows (CORE OF MVP) You will build only 3 workflows:
...
>>>>>>>> Open browser >>>>>>>> Navigate to GitHub >>>>>>>> Search query >>>>>>>> Filter results< >>>>>>>> >>>>>>>